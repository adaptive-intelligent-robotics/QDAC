...
class Walker2dFriction(PipelineEnv):
  ...
  def __init__(
    ...
    """
    print(f"[INFO] using friction index {friction_index} for walker2dfriction.xml")
    if friction_index is None:
      path = epath.resource_path('dreamerv3') / 'embodied/envs/walker2dfriction.xml'
    else:
      path = epath.resource_path('dreamerv3') / 'embodied/envs/walker2dfriction_{}.xml'.format(friction_index)
    sys = mjcf.load(path)
