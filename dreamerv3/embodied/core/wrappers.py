...
class ActionRepeat(base.Wrapper):
  ...
  def step(self, action):
    if action['reset']:
      return self.env.step(action)
    reward = 0.0
    for index_repeat in range(self._repeat):
      obs = self.env.step(action)
      if index_repeat == 0:
        feat = obs['feat']
      reward += obs['reward']
...
class ActuatorFailure(base.Wrapper):

  def __init__(self, env, env_name, actuator_failure_idx: List, actuator_failure_strength: List):
    print(f'ActuatorFailure: {env_name} actuator_failure_idx: {actuator_failure_idx} actuator_failure_strength: {actuator_failure_strength}')
    super().__init__(env)
    self._actuator_healthy = jnp.ones((env.action_size,)).at[jnp.array(actuator_failure_idx, dtype=jnp.int32)].set(jnp.array(actuator_failure_strength))

  def step(self, state: State, action: jnp.ndarray) -> State:
    state = self.env.step(state, action * self._actuator_healthy)
    return state
