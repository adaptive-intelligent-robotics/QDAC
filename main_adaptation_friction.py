...
from baselines.qdax.types import EnvState, RNGKey, Params, Observation, Action
...
from utils.eval_ppga_utils import load_repertoire_ppga, reevaluate_ppga_archive
...
task_folder_name = "walker2d_feet_contact"
...
def eval_ours(run_path, friction_index):
    ...
    # Create environment
    env = get_env(config, mode="train", friction_index=friction_index)
...
def eval_smerl(run_path, friction_index):
    ...
    # Init environment
    env = environments.create(config.task + "friction" + "_" + config.feat,
...
def eval_smerl_reverse(run_path, friction_index):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_uvfa(run_path, friction_index):
...
def eval_dcg_me(run_path, friction_index):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_qd_pg(run_path, friction_index):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_ppga(run_path, friction_index):
    with open(run_path / ".hydra" / "config.yaml") as f:
    ...
    cfg.env_batch_size = 1 * solution_batch_size
    vec_env = make_vec_env_brax_ppga(task_name=hydra_config.task + "friction", feat_name=hydra_config.feat,
...
def eval_domino(run_path, friction_index):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_scopa(run_path, friction_index):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_smerl_full_state(run_path, friction_index, is_reverse=False):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
    ...
    # random_key, subkey = jax.random.split(random_key)
    # fake_obs = jnp.zeros(shape=(env.observation_size,))
    # fake_desc = jnp.zeros(shape=(num_skills,))
    # fake_actor_params = actor_dc_network.init(subkey, fake_obs, fake_desc)
    ...
    # Create grid
    resolution = 50
    def play_step_actor_dc_fn(env_state, actor_dc_params, skill, random_key):
        combined_obs = jnp.concatenate([env_state.obs, skill], axis=-1)
        actions, random_key = smerl.select_action(
            obs=combined_obs,
            policy_params=actor_dc_params,
            random_key=random_key,
            deterministic=True,
        )
        state_desc = env_state.info["feat"]
        ...
        transition = QDTransition(
            ...
            next_state_desc=next_state.info["feat"],
            desc=jnp.zeros(env.behavior_descriptor_length, ) * jnp.nan,
            desc_prime=skill / env.behavior_descriptor_limits[1][0],
        )

        return next_state, actor_dc_params, skill, random_key, transition
    ...
    @jax.jit
    def evaluate_actor(random_key, params, goals):
        params = jax.tree_util.tree_map(lambda x: jnp.repeat(jnp.expand_dims(x, axis=0), goals.shape[0], axis=0),
                                        params)
        fitnesses, descriptors, extra_scores, random_key = scoring_fn(
            params, goals, random_key
        )
    ...
    fitnesses_list = []
    descriptor_list = []
    skills = jnp.eye(num_skills)

    for _ in range(n_visits_per_goal):
        fitnesses, descriptors, extra_scores, random_key = evaluate_actor(random_key, actor_params, skills)
        fitnesses_list.append(fitnesses)
        descriptor_list.append(descriptors)
    ...
    analysis_repertoire = AnalysisRepertoire(
        centroids=skills,
        fitnesses=jnp.stack(fitnesses_list, axis=1),
        descriptors=skills)  # returning skills as it does not make sense (and it does not work) to return the descriptors for full_state.
    return analysis_repertoire
...
df = pd.DataFrame(columns=["algo", "seed", "index", "fitness", "distance_to_goal"])

for j in range(1, n_index + 1):
    print(f"seed: {seed}, j: {j}, {j}/{n_index}")
    print(f"{algo}: {j}")
    analysis_repertoire = eval_fn(path, j)
    df.loc[len(df)] = [algo, seed, j, jnp.max(jnp.median(analysis_repertoire.fitnesses, axis=-1)), jnp.mean(-jnp.linalg.norm(analysis_repertoire.centroids - jnp.mean(analysis_repertoire.descriptors, axis=1), axis=-1))]

    path_csv = os.path.join(
        path,
        f"friction_{algo}_{task}_{seed}_{n_index}_{n_visits_per_goal}.csv"
    )
    df.to_csv(path_csv)
