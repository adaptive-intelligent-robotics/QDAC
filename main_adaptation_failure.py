...
from baselines.qdax.types import EnvState, RNGKey, Observation, Params, Action
...
task_folder_name = "humanoid_feet_contact"
...
def eval_ours(run_path, actuator_failure_idx, actuator_failure_strength):
    ...
    goal_sampler_cyclic = GoalSamplerCyclic(feat_space=env.feat_space,
                                            ...
                                            number_visits_per_goal=n_visits_per_skill)
...
def eval_smerl(run_path, actuator_failure_idx, actuator_failure_strength):
    ...
    fitnesses_list = []
    descriptor_list = []
    for _ in range(n_visits_per_skill):
...
def eval_qd_pg(run_path, actuator_failure_idx, actuator_failure_strength):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_ppga(run_path, actuator_failure_idx, actuator_failure_strength):
    with open(run_path / ".hydra" / "config.yaml") as f:
    ...
    vec_env = make_vec_env_brax_ppga(task_name=hydra_config.task + "failure", feat_name=hydra_config.feat, batch_size=cfg.env_batch_size,
    ...
    analysis_repertoire = reevaluate_ppga_archive(cfg, hydra_config, repertoire_ppga, vec_env, solution_batch_size, n_visits_per_skill, centroids, transform_descs_fn=transform_descs_fn)  # TODO
...
def eval_scopa(run_path, actuator_failure_idx, actuator_failure_strength):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_domino(run_path, actuator_failure_idx, actuator_failure_strength):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
for j, actuator_failure_strength in enumerate(jnp.linspace(0, 1, n_actuator_failure)):
    print(f"seed: {seed}, actuator_failure_strength: {j}/{n_actuator_failure}")
    print(f"{algo}: {actuator_failure_strength}")
    analysis_repertoire = eval_fn(path, actuator_failure_idx, [actuator_failure_strength])
    df.loc[len(df)] = [algo, seed, actuator_failure_strength, jnp.max(jnp.median(analysis_repertoire.fitnesses, axis=-1)), jnp.mean(-jnp.linalg.norm(analysis_repertoire.centroids - jnp.mean(analysis_repertoire.descriptors, axis=1), axis=-1))]

    path_csv = os.path.join(
        path,
        f"{task}_{actuator_name}_{algo}_{seed}_{n_actuator_failure}_{n_visits_per_skill}.csv"
    )
    df.to_csv(path_csv)
