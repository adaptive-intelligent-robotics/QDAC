...
from baselines.qdax.types import RNGKey, EnvState, Observation, Params, Action
...
task_folder_name = "humanoid_jump"
...
def eval_qd_pg(run_path, index, random_key):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
    ...
    result_repertoire = AnalysisRepertoire(
        centroids=goals,
        fitnesses=jnp.stack(fitnesses_list, axis=1),
        descriptors=jnp.stack(descriptor_list, axis=1))
    return result_repertoire




def eval_ppga(run_path, index, random_key):
    with open(run_path / ".hydra" / "config.yaml") as f:
    ...
    vec_env = make_vec_env_brax_ppga(task_name=hydra_config.task + "hurdle", feat_name=hydra_config.feat,
...
def eval_domino(run_path, index, random_key):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
def eval_scopa(run_path, index, random_key):
    with open(run_path / ".hydra" / "config.yaml") as f:
        config = yaml.safe_load(f)
    config = OmegaConf.load(run_path / ".hydra" / "config.yaml")
...
for j, height in enumerate(jnp.linspace(0, 0.5, 20)):
    ...
    print(f"seed: {seed}, {j}/{n_actuator_failure}")
    print(f"{algo}: {height}")
    random_key, subkey = jax.random.split(random_key)
    analysis_repertoire = eval_fn(path, j, subkey)
    df.loc[len(df)] = [algo, seed, height, jnp.max(jnp.median(analysis_repertoire.fitnesses, axis=-1)), jnp.mean(-jnp.linalg.norm(analysis_repertoire.centroids - jnp.mean(analysis_repertoire.descriptors, axis=1), axis=-1))]

    path_csv = os.path.join(
        path,
        f"humanoidhurdle_{algo}_{seed}_{n_actuator_failure}_{n_visits_per_goal}.csv"
    )
    df.to_csv(path_csv)
